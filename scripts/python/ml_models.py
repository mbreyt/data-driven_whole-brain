...
class TVB(nn.Module):
    ...
    def fwd(self, nmm, region_pars, g):
        def tvb_dfun(buf, x, t, stim):
            ...
            coupled_x = self.tvb_p['W']@buf[t - 1, :, :self.nst_vars]
            # jax.debug.print('coupled_x {x}', x=coupled_x.shape)
    ...
    @nn.compact
    def __call__(self, region_pars, g, sim_len=0, seed=42, initial_cond=False, mlp=True, stimulus_yn=True):
        # if inputs==None:
